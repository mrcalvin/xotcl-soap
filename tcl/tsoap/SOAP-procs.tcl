...
catch {namespace import -force ::rpcvar::*}
...
namespace eval ::SOAP {
    ...
    variable rcs_version { $Id: SOAP-procs.tcl,v 1.2 2005/03/23 22:43:44 jeffd Exp $ }
...
proc ::SOAP::invoke { procVarName args } {
    ...
    upvar $procVarName procvar
ns_log notice "HERE: $procVarName"
    # Get the URL
...
proc ::SOAP::reply { doc uri methodName result } {
    ...
    if {$result != {}} {
        ...
        if {[::rpcvar::rpctype $result] == "PARAMLIST"} {
            foreach {resultName resultValue} [::rpcvar::rpcvalue $result] {
...
#    set xml [regsub -all {SOAP-ENV:|ns:} $xml {}]
#    ns_log notice $xml
    ...
    $doc delete
ns_log notice soap_parse_response:$result
    return $result
}
...
proc ::SOAP::insert_value {node value} {
    ...
    set subtype  [::rpcvar::rpcsubtype $value]
    set attrs    [::rpcvar::rpcattributes $value]
    set headers  [::rpcvar::rpcheaders $value]
    set value    [::rpcvar::rpcvalue $value]
    set typeinfo [::rpcvar::typedef -info $type]
    set typexmlns [::rpcvar::typedef -namespace $type]
